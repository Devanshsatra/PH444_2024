# %% yz oscillating charge Ex, Ey, Ez, Bx, By, Bz
import numpy as np
...
ims[9].set_data(B_total[1].T)
ims[10].set_data(B_vel[1].T)
ims[11].set_data(B_acc[1].T)
ims[12].set_data(B_total[2].T)
ims[13].set_data(B_vel[2].T)
ims[14].set_data(B_acc[2].T)
...
for i, label in zip((2, 5, 8, 11, 14), ('$E_x$ [N/C]', '$E_y$ [N/C]', '$E_z$ [N/C]', '$B_y$ [T]', '$B_z$ [T]')):
