...
charges = (OrbittingCharge(pos_charge=True, phase=0, max_speed=0.5*c),OrbittingCharge(pos_charge=False, phase=np.pi, max_speed=0.5*c))
t = 0
...
# %% Animation
charges = (OrbittingCharge(pos_charge=True, phase=0, max_speed=0.5*c, start_zero=True),OrbittingCharge(pos_charge=False, phase=np.pi, max_speed=0.5*c, start_zero=True))
...
end_time = time.time()
print(f"Time taken to execute 1st Section: {end_time - start_time} seconds")
...
ims[4].set_data(E_vel[1].T)
ims[5].set_data(E_acc[1].T)
ims[6].set_data(B_total[2].T)
ims[7].set_data(B_vel[2].T)
ims[8].set_data(B_acc[2].T)
...
ani.save(os.path.dirname(os.path.realpath(__file__))+'/Animations/'+savename+'.mp4',
         writer=animation.FFMpegWriter(fps=24), dpi=500)
end_time = time.time()
print(f"Time taken to execute 2nd Section: {end_time - start_time} seconds")