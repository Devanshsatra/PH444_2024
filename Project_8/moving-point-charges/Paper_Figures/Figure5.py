# %% yz oscillating dipole Ex, By, Bz
import numpy as np
...
t = 0
X, Y, Z = np.meshgrid(0, np.linspace(-lim, lim, grid_size),
...
for i, ax in enumerate(axes):
    ...
    ims[i].set_extent((1e9*Y[0, 0, 0], 1e9*Y[0, -1, 0],
...
axes[7].set_xlabel('$y$ [nm]')

ims[0].set_data(E_total[0].T)
ims[1].set_data(E_vel[0].T)
ims[2].set_data(E_acc[0].T)
ims[3].set_data(B_total[1].T)
ims[4].set_data(B_vel[1].T)
ims[5].set_data(B_acc[1].T)
...
for i, label in zip((2, 5, 8), ('$E_x$ [N/C]', '$B_y$ [T]', '$B_z$ [T]')):
    Bcax = inset_axes(axes[i],
                      width="6%",
                      height="100%",
                      loc='lower left',
    ...
    B_cbar = plt.colorbar(ims[i], cax=Bcax)
    B_cbar.ax.set_ylabel(label, rotation=270, labelpad=12)
